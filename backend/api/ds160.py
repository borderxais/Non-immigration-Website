...
from flask import request, make_response, Blueprint, jsonify, current_app, send_file
...
@api.route("/form/<string:application_id>/pdf")
class DS160FormPDFResource(Resource):
    @jwt_required()
    def get(self, application_id):
        """Generate a PDF for a DS-160 form"""
        try:
            # Find the translation
            translation = DS160FormTranslation.query.filter_by(
                original_form_application_id=application_id
            ...
            # Add the title
            elements.append(Paragraph(f"DS-160 Form: {application_id}", title_style))
            elements.append(Spacer(1, 0.25*inch))
            
            # Add creation date
            creation_date = translation.created_at.strftime("%Y-%m-%d %H:%M:%S") if translation.created_at else "Unknown"
            elements.append(Paragraph(f"Created: {creation_date}", normal_style))
            elements.append(Spacer(1, 0.25*inch))
            
            # Process form data
            if translation.form_data:
                form_data = translation.form_data
                
                # Define the sections to include in the PDF
                sections = [
                    {"key": "personalInfo1", "title": "Personal Information I"},
                    {"key": "personalInfo2", "title": "Personal Information II"},
                    {"key": "travelInfo", "title": "Travel Information"},
                    {"key": "passport", "title": "Passport Information"},
                    {"key": "addressAndPhone", "title": "Address and Phone"},
                    {"key": "familyRelatives", "title": "Family Information"},
                    {"key": "workEducation", "title": "Work and Education"}
                ]
                
                # Add each section to the PDF
                for section in sections:
                    section_data = form_data.get(section["key"])
                    if section_data:
                        # Add section title
                        elements.append(Paragraph(section["title"], heading_style))
                        elements.append(Spacer(1, 0.1*inch))
                        
                        # Create a table for this section's data
                        data = []
                        for key, value in section_data.items():
                            # Skip internal fields
                            if key.startswith('_') or key.endswith('_na'):
                                continue
                            ...
                            if isinstance(value, dict):
                                # Handle date objects or nested data
                                if 'day' in value and 'month' in value and 'year' in value:
                                    formatted_value = f"{value.get('day', '')}-{value.get('month', '')}-{value.get('year', '')}"
                                else:
                                    formatted_value = ', '.join([f"{k}: {v}" for k, v in value.items() if not k.startswith('_')])
                            ...
                            data.append([field_name, formatted_value])
                ...
                elements.append(Paragraph("Form Status", heading_style))
                elements.append(Spacer(1, 0.1*inch))
                
                status_data = [
                    ["Status", translation.status.capitalize() if translation.status else "Unknown"],
                    ["Last Updated", translation.updated_at.strftime("%Y-%m-%d %H:%M:%S") if translation.updated_at else "Unknown"]
                ]
            ...
            return send_file(
                ...
                as_attachment=True,
                download_name=f"DS160-{application_id}.pdf"
            )
